...
def start(update, context):
    ...
    buttons.buildbutton("Contact", "https://t.me/b4dm4n_bot")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f'''
Yes, Badman is now alive.
Type /{BotCommands.HelpCommand} to get a list of available commands
'''
        sendMarkup(start_string, context.bot, update, reply_markup)
    else:
        sendMarkup(
            'Sorry, Authorized User.',
            context.bot,
            update,
            reply_markup,
        )
...
help = Telegraph(access_token=telegraph_token).create_page(
        title='Badman Commands',
        author_name='Badman',
        author_url='https://t.me/b4dm4n_bot',
...
def main():
    ...
    if os.path.isfile(".restartmsg"):
        ...
        bot.edit_message_text("Badman Restarted successfully!", chat_id, msg_id)
        os.remove(".restartmsg")
    elif OWNER_ID:
        try:
            text = "<b>Badman Restarted!</b>"
